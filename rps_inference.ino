// Use of Edge Impulse code and adafruit pwm servo driver example code.   
...
/* Includes ---------------------------------------------------------------- */
#include <rock-paper-scissors_inferencing.h>
...
void rock() {
  // Move servos to make rock gesture
  setServoAngle(0, 0);  // Closed thumb
  setServoAngle(1, 90); // Closed index
  setServoAngle(2, 90); // Closed middle
  setServoAngle(3, 90); // Closed ring
  setServoAngle(4, 90); // Closed pinky
  Serial.println("Rock");
}

void paper() {
  // Move servos to make paper gesture
  setServoAngle(0, 50); // Open thumb
  setServoAngle(1, 0);  // Open index
  setServoAngle(2, 0);  // Open middle
  setServoAngle(3, 0);  // Open ring
  setServoAngle(4, 0);  // Open pinky
  Serial.println("Paper");
}

void scissors() {
  // Move servos to make scissors gesture
  setServoAngle(0, 0); // Open thumb
  setServoAngle(1, 0);  // Open index
  setServoAngle(2, 0);  // Open middle
  setServoAngle(3, 90); // Closed ring
  setServoAngle(4, 90); // Closed pinky
...
void loop()
{
    ...
    uint16_t highest_prob_index = 0;
    float highest_prob = result.classification[0].value;
    ...
    // Print the predicted class with the highest probability
    const char* predicted_class = ei_classifier_inferencing_categories[highest_prob_index];
    ei_printf("Predicted class: %s (%.5f)\n", predicted_class, highest_prob);

    // Run the corresponding function based on the predicted class
    if (strcmp(predicted_class, "rock") == 0) {
        rock();
    }
    else if (strcmp(predicted_class, "paper") == 0) {
        paper();
    }
    else if (strcmp(predicted_class, "scissors") == 0) {
        scissors();
    }
}
